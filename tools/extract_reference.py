#!/usr/bin/env python3
"""Extract and repair the standalone design bundle from a saved JustPaste page."""
...
def repair_source(source: str) -> str:
    ...
    template = template.replace(
        "<html><head>\n<meta charset=\"utf-8\">",
        "<html><head>\n<meta charset=\"utf-8\">\n"
        "<title>Disin · Tactile player reference</title>",
    )
    template = template.replace(
        "body{margin:0;background:#26211c;display:flex;justify-content:center;padding:48px 0}",
        "body{margin:0;background:#26211c;display:flex;justify-content:center;padding:14px 0}",
    )
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("usage: extract_reference.py INPUT OUTPUT")

    input_path = Path(sys.argv[1])
    output_path = Path(sys.argv[2])
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(
        repair_source(extract_source(input_path.read_text(encoding="utf-8"))),
        encoding="utf-8",
    )
